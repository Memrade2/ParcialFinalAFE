<template>
  <div class="container">
    ...
    <div v-else>
      <add-seller v-on:reloadlist="getList()" />
    </div>

<!-- Traemos la view de la data -->
    <list-view-seller class="pt-3"
      :sellers="sellers"
      v-on:reloadlist="getList()"
      v-on:reloadedit="loadEdit"
    />
...
<script>
import addSeller from "./addSeller.vue";
import editSeller from "./editSeller.vue";
import listViewSeller from "./listViewSeller.vue";
...
computed: {
  ...
  methods: {
    getList(page) {
      var urlSellers = "api/sellers?page=" + page;
      axios
        .get(urlSellers)
        .then((response) => {
